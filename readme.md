...
#### ⚠️ 注意事项：Anthropic 协议下输出「突然截断」

Anthropic 协议的 `max_tokens` 由程序写死为 **8192**，而且**思考的 token 与正文、工具调用参数共用这一个额度**。一旦撞上它，就会出现这些症状：

- 回复写到一半突然停住；
- 模型说要调用 `compress` / `write_full_file` / `edit_file_match` 这类**参数很长**的工具，结果**说要做却没做**，控制台也不报错。

之所以难查，是因为截断是**全程静默**的：`stop_reason == "max_tokens"` 不会提示，工具参数 JSON 被截断后解析失败也会被直接跳过。**看不到报错 ≠ 没有出错。**

解决办法就是**把 `max_tokens` 调大**（该字段目前通过 `extra_body` 配置，`max_tokens` 不在结构保护名单里，正是为了留这个口子）：

```jsonc
{ "extra_body": { "max_tokens": 32768 } }
```

如果开了扩展思考，`max_tokens` 要**大于** `budget_tokens`，并给正文和工具参数留够空间：
...
调大没有副作用：它只是**单次调用的输出上限**，与上下文窗口（`context_limit_tokens`）无关，也不产生额外费用（上限不是预留，写完就停）。只需注意两点——别超过模型自身的最大输出能力，也别让「输入 + `max_tokens`」超出服务端的上下文限制（部分部署会校验并返回 400）。

> OpenAI 协议不受此影响：程序不发送 `max_tokens`，输出上限由服务端决定。
