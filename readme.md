...
```
📊 上下文 12,345 tok / 1,000,000（1.2%） ｜ 本次输出 345 tok（合计 12,690） ｜ 字节 51,200 B（50.0 KB） ｜ 约 4.15 B/tok
```

- **token 数取自 API 返回的真实 `usage`**（上下文按上次请求的 token / 字节比例换算，每次 API 返回都会自我校正），不再是拍脑袋的字节估算；
- 同一行里给出该上下文的 **JSON 字节数**及换算比 `约 x B/tok`，两者对照一目了然；
